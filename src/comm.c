...
int comm_init() { return xs_init(); }

int comm_clean() { return xs_clean(); }
...
int comm_close_socket(xs_SOCKET sockfd) { return xs_close(sockfd); }
...
xs_SOCKET comm_connect_server(const char *hostname, int port) {
  ...
  for (rp = result; rp != NULL; rp = rp->ai_next) {
    ...
    if (connect(sock, rp->ai_addr, (int)rp->ai_addrlen) != xs_ERROR) {
      clog_i(_COMM, "connection established successfully to %s on port %d",
             hostname, port);
      break;
    }
    xs_close(sock);
